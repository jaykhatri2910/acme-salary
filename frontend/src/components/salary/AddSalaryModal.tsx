...
const addSalarySchema = z.object({
  ...
  effectiveDate: z
    ...
    .refine((v) => v <= todayYMD(), "Effective date cannot be in the future"),
...
export const AddSalaryModal: React.FC<AddSalaryModalProps> = ({
  ...
  const { mutate, isPending, isSuccess, isError, reset } =
    useAddSalary(employeeId);
  const [apiFieldErrors, setApiFieldErrors] = React.useState<
  ...
  const {
    ...
    // Dynamically update form values when props change
    values: {
        ...
        <>
          <div className="flex items-start gap-3 rounded-lg bg-amber-500/10 border border-amber-500/25 p-4 mb-5">
            ...
            <p className="text-xs text-amber-300 leading-relaxed">
              This will create a <strong>permanent, immutable</strong> salary
              record for <strong>{employeeName}</strong>. Existing records
              cannot be edited or deleted. This action cannot be undone.
            </p>
          </div>
          ...
          >
            ...
            {isError && Object.keys(apiFieldErrors).length === 0 && (
              <div className="flex items-start gap-2 rounded-lg bg-destructive/10 border border-destructive/25 p-3">
                ...
                <p className="text-xs text-destructive">
                  Failed to record salary change. Please check your inputs and
                  try again.
                </p>
              </div>
            )}
