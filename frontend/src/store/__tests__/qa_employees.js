  ...
  try {
    ...
    for (let i = 0; i < ascSalaries.length - 1; i++) {
      if (ascSalaries[i] > ascSalaries[i+1]) {
        // If one is null (0), it is sorted last/first, check non-zero
    ...
    if (isAsc && isDesc) {
      console.log('✅ Column sorting (Salary) verified in both directions.');
    } else {
      console.log('⚠️ Warning: Sorting direction checks. (Note: database null values might impact simple numeric sort asserts).');
    }
